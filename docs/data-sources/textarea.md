...
page_title: "html_textarea Data Source - terraform-provider-html"
subcategory: ""
description: |-
...
- `autocapitalize` (Dynamic) Controls whether inputted text is automatically capitalized and, if so, in what manner.
- `autocomplete` (Dynamic) This attribute indicates whether the value of the control can be automatically completed by the browser.
- `autofocus` (Dynamic) This Boolean attribute lets you specify that a form control should have input focus when the page loads.
...
- `cols` (Dynamic) The visible width of the text control, in average character widths.
...
- `dirname` (Dynamic) This attribute is used to indicate the text directionality of the element contents.
- `disabled` (Dynamic) This Boolean attribute indicates that the user cannot interact with the control.
...
- `form` (Dynamic) The form element that the `<textarea>` element is associated with (its "form owner").
...
- `maxlength` (Dynamic) The maximum string length (measured in UTF-16 code units) that the user can enter.
- `minlength` (Dynamic) The minimum string length (measured in UTF-16 code units) required that the user should enter.
- `name` (Dynamic) The name of the control.
...
- `placeholder` (Dynamic) A hint to the user of what can be entered in the control.
...
- `readonly` (Dynamic) This Boolean attribute indicates that the user cannot modify the value of the control.
- `required` (Dynamic) This attribute specifies that the user must fill in a value before submitting a form.
- `rows` (Dynamic) The number of visible text lines for the control.
...
- `spellcheck` (Dynamic) Specifies whether the `<textarea>` is subject to spell checking by the underlying browser/OS.
...
- `wrap` (Dynamic) Indicates how the control should wrap the value for form submission.
