...
page_title: "html_form Data Source - terraform-provider-html"
subcategory: ""
description: |-
...
- `accept_charset` (Dynamic) Space-separated [character encodings](https://developer.mozilla.org/en-US/docs/Glossary/Character_encoding) the server accepts.
...
- `action` (Dynamic) The URL that processes the form submission.
- `autocapitalize` (Dynamic) Controls whether inputted text is automatically capitalized and, if so, in what manner.
- `autocomplete` (Dynamic) Indicates whether input elements can by default have their values automatically completed by the browser.
...
- `enctype` (Dynamic) If the value of the `method` attribute is `post`, `enctype` is the [MIME type](https://en.wikipedia.org/wiki/Mime_type) of the form submission.
...
- `method` (Dynamic) The [HTTP](https://developer.mozilla.org/en-US/docs/Web/HTTP) method to submit the form with.
- `name` (Dynamic) The name of the form. The value must not be the empty string, and must be unique among the `form` elements in the forms collection that it is in, if any.
...
- `novalidate` (Dynamic) This Boolean attribute indicates that the form shouldn't be validated when submitted.
...
- `rel` (Dynamic) Controls the annotations and what kinds of links the form creates.
...
- `target` (Dynamic) Indicates where to display the response after submitting the form. It is a name/keyword for a _browsing context_ (for example, tab, window, or iframe).
