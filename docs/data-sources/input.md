...
page_title: "html_input Data Source - terraform-provider-html"
subcategory: ""
description: |-
  The <input> HTML https://developer.mozilla.org/en-US/docs/Web/HTML element is used to create interactive controls for web-based forms in order to accept data from the user.
  For more information, see the documentation https://developer.mozilla.org/en-US/docs/Web/HTML/Element/input.
---

# html_input (Data Source)
...
- `accept` (Dynamic) Hint for expected file type in file upload controls.
...
- `alt` (Dynamic) alt attribute for the image type.
- `autocapitalize` (Dynamic) Controls automatic capitalization in inputted text.
- `autocomplete` (Dynamic) Hint for form autofill feature.
...
- `capture` (Dynamic) Media capture input method in file upload controls.
- `checked` (Dynamic) Whether the command or control is checked.
...
- `dirname` (Dynamic) Name of form field to use for sending the element's directionality in form submission.
- `disabled` (Dynamic) Whether the form control is disabled.
...
- `form` (Dynamic) Associates the control with a form element.
- `formaction` (Dynamic) URL to use for form submission.
- `formenctype` (Dynamic) Form data set encoding type to use for form submission.
- `formmethod` (Dynamic) HTTP method to use for form submission.
- `formnovalidate` (Dynamic) Bypass form control validation for form submission.
- `formtarget` (Dynamic) Browsing context for form submission.
- `height` (Dynamic) Same as height attribute for [`<img>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/img).
...
- `list` (Dynamic) Value of the id attribute of the [`<datalist>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/datalist) of autocomplete options.
- `max` (Dynamic) Maximum value.
- `maxlength` (Dynamic) Maximum length (number of characters) of `value`.
- `min` (Dynamic) Minimum value.
- `minlength` (Dynamic) Minimum length (number of characters) of `value`.
- `multiple` (Dynamic) Whether to allow multiple values.
- `name` (Dynamic) Name of the form control.
...
- `pattern` (Dynamic) Pattern the `value` must match to be valid.
- `placeholder` (Dynamic) Text that appears in the form control when it has no value set.
...
- `popovertarget` (Dynamic) Designates an `<input type="button">` as a control for a popover element.
- `popovertargetaction` (Dynamic) Specifies the action that a popover control should perform.
- `readonly` (Dynamic) The value is not editable.
- `required` (Dynamic) A value is required or must be checked for the form to be submittable.
- `size` (Dynamic) Size of the control.
...
- `src` (Dynamic) Same as `src` attribute for [`<img>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/img).
- `step` (Dynamic) Incremental values that are valid.
...
- `type` (Dynamic) Type of form control.
- `value` (Dynamic) The value of the control.
- `width` (Dynamic) Same as `width` attribute for [`<img>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/img).
