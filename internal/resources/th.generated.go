...
var (
	_ resource.Resource = &resource_th{}
)

func newResource_th() resource.Resource {
	return &resource_th{}
}

type resource_th struct{}

func (r *resource_th) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_th"
}

func (r *resource_th) Schema(_ context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "The **`<th>`** [HTML](https://developer.mozilla.org/en-US/docs/Web/HTML) element defines a cell as the header of a group of table cells and may be used as a child of the [`<tr>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/tr) element.\n\nFor more information, see the [documentation](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/th).",
		Attributes: map[string]schema.Attribute{
			...
			"abbr": schema.DynamicAttribute{
				MarkdownDescription: "A short, abbreviated description of the header cell's content provided as an alternative label to use for the header cell when referencing the cell in other contexts.",
				Optional:            true,
			},
			"colspan": schema.DynamicAttribute{
				MarkdownDescription: "A non-negative integer value indicating how many columns the header cell spans or extends.",
				Optional:            true,
			},
			"headers": schema.DynamicAttribute{
				MarkdownDescription: "A list of space-separated strings corresponding to the `id` attributes of the `<th>` elements that provide the headers for this header cell.",
				Optional:            true,
			},
			"rowspan": schema.DynamicAttribute{
				MarkdownDescription: "A non-negative integer value indicating how many rows the header cell spans or extends.",
				Optional:            true,
			},
			"scope": schema.DynamicAttribute{
				MarkdownDescription: "Defines the cells that the header (defined in the `<th>`) element relates to.",
				Optional:            true,
			},
...
type resource_thModel struct {
	Children        types.List    `tfsdk:"children"`
	Abbr            types.Dynamic `tfsdk:"abbr"`
	Colspan         types.Dynamic `tfsdk:"colspan"`
	Headers         types.Dynamic `tfsdk:"headers"`
	Rowspan         types.Dynamic `tfsdk:"rowspan"`
...
func (r *resource_th) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	r.handleRequest(ctx, &req.Plan, &resp.State, &resp.Diagnostics)
}

func (r *resource_th) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	r.handleRequest(ctx, &req.State, &resp.State, &resp.Diagnostics)
}

func (r *resource_th) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	r.handleRequest(ctx, &req.Plan, &resp.State, &resp.Diagnostics)
}

func (r *resource_th) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
}

func (r *resource_th) handleRequest(ctx context.Context, g util.ModelGetter, s util.ModelSetter, diags *diag.Diagnostics) {
	util.HandleRequest(
		ctx,
		&resource_thModel{},
		g,
		...
		func(m *resource_thModel) bool {
			...
			attrs := []string{}
			if !m.Abbr.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "abbr", m.Abbr)
				if err != nil {
					diags.AddError("invalid abbr attribute", err.Error())
					return false
				}
				attrs = append(attrs, attr)
			}
			if !m.Colspan.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "colspan", m.Colspan)
				if err != nil {
					diags.AddError("invalid colspan attribute", err.Error())
					return false
				}
				attrs = append(attrs, attr)
			}
			if !m.Headers.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "headers", m.Headers)
				if err != nil {
					diags.AddError("invalid headers attribute", err.Error())
					return false
				}
				attrs = append(attrs, attr)
			}
			if !m.Rowspan.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "rowspan", m.Rowspan)
				if err != nil {
					diags.AddError("invalid rowspan attribute", err.Error())
					return false
				}
				attrs = append(attrs, attr)
			}
			if !m.Scope.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "scope", m.Scope)
				if err != nil {
					diags.AddError("invalid scope attribute", err.Error())
					return false
				}
				attrs = append(attrs, attr)
			}
