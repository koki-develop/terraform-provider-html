...
var (
	_ resource.Resource = &resource_q{}
)

func newResource_q() resource.Resource {
	return &resource_q{}
}

type resource_q struct{}

func (r *resource_q) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_q"
}

func (r *resource_q) Schema(_ context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "The **`<q>`** [HTML](https://developer.mozilla.org/en-US/docs/Web/HTML) element indicates that the enclosed text is a short inline quotation.\n\nFor more information, see the [documentation](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/q).",
		Attributes: map[string]schema.Attribute{
			...
			"cite": schema.DynamicAttribute{
				MarkdownDescription: "The value of this attribute is a URL that designates a source document or message for the information quoted.",
				Optional:            true,
			},
...
func (r *resource_q) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	r.handleRequest(ctx, &req.Plan, &resp.State, &resp.Diagnostics)
}

func (r *resource_q) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	r.handleRequest(ctx, &req.State, &resp.State, &resp.Diagnostics)
}

func (r *resource_q) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	r.handleRequest(ctx, &req.Plan, &resp.State, &resp.Diagnostics)
}

func (r *resource_q) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
}

func (r *resource_q) handleRequest(ctx context.Context, g util.ModelGetter, s util.ModelSetter, diags *diag.Diagnostics) {
	util.HandleRequest(
		ctx,
		&resource_qModel{},
		g,
		...
		func(m *resource_qModel) bool {
			...
			attrs := []string{}
			if !m.Cite.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "cite", m.Cite)
				if err != nil {
					diags.AddError("invalid cite attribute", err.Error())
					return false
				}
