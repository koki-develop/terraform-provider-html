...
var (
	_ resource.Resource = &resource_col{}
)

func newResource_col() resource.Resource {
	return &resource_col{}
}

type resource_col struct{}

func (r *resource_col) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_col"
}

func (r *resource_col) Schema(_ context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "The **`<col>`** [HTML](https://developer.mozilla.org/en-US/docs/Web/HTML) element defines one or more columns in a column group represented by its parent [`<colgroup>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/colgroup) element.\n\nFor more information, see the [documentation](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/col).",
		Attributes: map[string]schema.Attribute{
			...
			"span": schema.DynamicAttribute{
				MarkdownDescription: "Specifies the number of consecutive columns the `<col>` element spans.",
				Optional:            true,
			},
...
func (r *resource_col) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	r.handleRequest(ctx, &req.Plan, &resp.State, &resp.Diagnostics)
}

func (r *resource_col) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	r.handleRequest(ctx, &req.State, &resp.State, &resp.Diagnostics)
}

func (r *resource_col) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	r.handleRequest(ctx, &req.Plan, &resp.State, &resp.Diagnostics)
}

func (r *resource_col) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
}

func (r *resource_col) handleRequest(ctx context.Context, g util.ModelGetter, s util.ModelSetter, diags *diag.Diagnostics) {
	util.HandleRequest(
		ctx,
		&resource_colModel{},
		g,
		...
		func(m *resource_colModel) bool {
			...
			attrs := []string{}
			if !m.Span.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "span", m.Span)
				if err != nil {
					diags.AddError("invalid span attribute", err.Error())
					return false
				}
