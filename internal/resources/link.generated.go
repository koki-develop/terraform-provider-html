...
var (
	_ resource.Resource = &resource_link{}
)

func newResource_link() resource.Resource {
	return &resource_link{}
}

type resource_link struct{}

func (r *resource_link) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_link"
}

func (r *resource_link) Schema(_ context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "The **`<link>`** [HTML](https://developer.mozilla.org/en-US/docs/Web/HTML) element specifies relationships between the current document and an external resource.\n\nFor more information, see the [documentation](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/link).",
		Attributes: map[string]schema.Attribute{
			...
			"as": schema.DynamicAttribute{
				MarkdownDescription: "This attribute is required when [`rel=\"preload\"`](https://developer.mozilla.org/en-US/docs/Web/HTML/Attributes/rel/preload) has been set on the `<link>` element, optional when [`rel=\"modulepreload\"`](https://developer.mozilla.org/en-US/docs/Web/HTML/Attributes/rel/modulepreload) has been set, and otherwise should not be used.",
				Optional:            true,
			},
			"crossorigin": schema.DynamicAttribute{
				MarkdownDescription: "This [enumerated](https://developer.mozilla.org/en-US/docs/Glossary/Enumerated) attribute indicates whether [CORS](https://developer.mozilla.org/en-US/docs/Glossary/CORS) must be used when fetching the resource.",
				Optional:            true,
			},
			"disabled": schema.DynamicAttribute{
				MarkdownDescription: "For `rel=\"stylesheet\"` only, the `disabled` Boolean attribute indicates whether the described stylesheet should be loaded and applied to the document.",
				Optional:            true,
			},
			"fetchpriority": schema.DynamicAttribute{
				MarkdownDescription: "Provides a hint of the relative priority to use when fetching a preloaded resource.",
				Optional:            true,
			},
			"href": schema.DynamicAttribute{
				MarkdownDescription: "This attribute specifies the [URL](https://developer.mozilla.org/en-US/docs/Glossary/URL) of the linked resource.",
				Optional:            true,
			},
			"hreflang": schema.DynamicAttribute{
				MarkdownDescription: "This attribute indicates the language of the linked resource.",
				Optional:            true,
			},
			"imagesizes": schema.DynamicAttribute{
				MarkdownDescription: "For `rel=\"preload\"` and `as=\"image\"` only, the `imagesizes` attribute is [a sizes attribute](https://html.spec.whatwg.org/multipage/images.html#sizes-attribute) that indicates to preload the appropriate resource used by an `img` element with corresponding values for its `srcset` and `sizes` attributes.",
				Optional:            true,
			},
			"imagesrcset": schema.DynamicAttribute{
				MarkdownDescription: "For `rel=\"preload\"` and `as=\"image\"` only, the `imagesrcset` attribute is [a sourceset attribute](https://html.spec.whatwg.org/multipage/images.html#srcset-attribute) that indicates to preload the appropriate resource used by an `img` element with corresponding values for its `srcset` and `sizes` attributes.",
				Optional:            true,
			},
			"integrity": schema.DynamicAttribute{
				MarkdownDescription: "Contains inline metadata — a base64-encoded cryptographic hash of the resource (file) you're telling the browser to fetch.",
				Optional:            true,
			},
			"media": schema.DynamicAttribute{
				MarkdownDescription: "This attribute specifies the media that the linked resource applies to.",
				Optional:            true,
			},
			...
			"rel": schema.DynamicAttribute{
				MarkdownDescription: "This attribute names a relationship of the linked document to the current document.",
				Optional:            true,
			},
			"sizes": schema.DynamicAttribute{
				MarkdownDescription: "This attribute defines the sizes of the icons for visual media contained in the resource.",
				Optional:            true,
			},
			"title": schema.DynamicAttribute{
				MarkdownDescription: "The `title` attribute has special semantics on the `<link>` element.",
				Optional:            true,
			},
			"type": schema.DynamicAttribute{
				MarkdownDescription: "This attribute is used to define the type of the content linked to.",
				Optional:            true,
			},
...
type resource_linkModel struct {
	Children        types.List    `tfsdk:"children"`
	As              types.Dynamic `tfsdk:"as"`
	...
	Imagesizes      types.Dynamic `tfsdk:"imagesizes"`
	Imagesrcset     types.Dynamic `tfsdk:"imagesrcset"`
...
func (r *resource_link) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	r.handleRequest(ctx, &req.Plan, &resp.State, &resp.Diagnostics)
}

func (r *resource_link) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	r.handleRequest(ctx, &req.State, &resp.State, &resp.Diagnostics)
}

func (r *resource_link) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	r.handleRequest(ctx, &req.Plan, &resp.State, &resp.Diagnostics)
}

func (r *resource_link) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
}

func (r *resource_link) handleRequest(ctx context.Context, g util.ModelGetter, s util.ModelSetter, diags *diag.Diagnostics) {
	util.HandleRequest(
		ctx,
		&resource_linkModel{},
		g,
		...
		func(m *resource_linkModel) bool {
			...
			attrs := []string{}
			if !m.As.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "as", m.As)
				if err != nil {
					diags.AddError("invalid as attribute", err.Error())
					return false
				}
			...
			if !m.Imagesizes.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "imagesizes", m.Imagesizes)
				if err != nil {
					diags.AddError("invalid imagesizes attribute", err.Error())
					return false
				}
				attrs = append(attrs, attr)
			}
			if !m.Imagesrcset.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "imagesrcset", m.Imagesrcset)
				if err != nil {
					diags.AddError("invalid imagesrcset attribute", err.Error())
					return false
				}
				attrs = append(attrs, attr)
			}
			...
			if !m.Media.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "media", m.Media)
				if err != nil {
					diags.AddError("invalid media attribute", err.Error())
					return false
				}
				attrs = append(attrs, attr)
			}
