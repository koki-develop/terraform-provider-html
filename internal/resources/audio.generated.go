...
var (
	_ resource.Resource = &resource_audio{}
)

func newResource_audio() resource.Resource {
	return &resource_audio{}
}

type resource_audio struct{}

func (r *resource_audio) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_audio"
}

func (r *resource_audio) Schema(_ context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "The **`<audio>`** [HTML](https://developer.mozilla.org/en-US/docs/Web/HTML) element is used to embed sound content in documents.\n\nFor more information, see the [documentation](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/audio).",
		Attributes: map[string]schema.Attribute{
			...
			"autoplay": schema.DynamicAttribute{
				MarkdownDescription: "A Boolean attribute: if specified, the audio will automatically begin playback as soon as it can do so, without waiting for the entire audio file to finish downloading.",
				Optional:            true,
			},
			"controls": schema.DynamicAttribute{
				MarkdownDescription: "If this attribute is present, the browser will offer controls to allow the user to control audio playback, including volume, seeking, and pause/resume playback.",
				Optional:            true,
			},
			"controlslist": schema.DynamicAttribute{
				MarkdownDescription: "The [`controlslist`](https://wicg.github.io/controls-list/explainer.html) attribute, when specified, helps the browser select what controls to show for the `audio` element whenever the browser shows its own set of controls (that is, when the `controls` attribute is specified).",
				Optional:            true,
			},
			"crossorigin": schema.DynamicAttribute{
				MarkdownDescription: "This [enumerated](https://developer.mozilla.org/en-US/docs/Glossary/Enumerated) attribute indicates whether to use CORS to fetch the related audio file.",
				Optional:            true,
			},
			...
			"loop": schema.DynamicAttribute{
				MarkdownDescription: "A Boolean attribute: if specified, the audio player will automatically seek back to the start upon reaching the end of the audio.",
				Optional:            true,
			},
			"muted": schema.DynamicAttribute{
				MarkdownDescription: "A Boolean attribute that indicates whether the audio will be initially silenced.",
				Optional:            true,
			},
			"preload": schema.DynamicAttribute{
				MarkdownDescription: "This [enumerated](https://developer.mozilla.org/en-US/docs/Glossary/Enumerated) attribute is intended to provide a hint to the browser about what the author thinks will lead to the best user experience.",
				Optional:            true,
			},
			"src": schema.DynamicAttribute{
				MarkdownDescription: "The URL of the audio to embed.",
				Optional:            true,
			},
...
type resource_audioModel struct {
...
func (r *resource_audio) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	r.handleRequest(ctx, &req.Plan, &resp.State, &resp.Diagnostics)
}

func (r *resource_audio) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	r.handleRequest(ctx, &req.State, &resp.State, &resp.Diagnostics)
}

func (r *resource_audio) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	r.handleRequest(ctx, &req.Plan, &resp.State, &resp.Diagnostics)
}

func (r *resource_audio) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
}

func (r *resource_audio) handleRequest(ctx context.Context, g util.ModelGetter, s util.ModelSetter, diags *diag.Diagnostics) {
	util.HandleRequest(
		ctx,
		&resource_audioModel{},
		g,
