...
var (
	_ resource.Resource = &resource_form{}
)

func newResource_form() resource.Resource {
	return &resource_form{}
}

type resource_form struct{}

func (r *resource_form) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_form"
}

func (r *resource_form) Schema(_ context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "The **`<form>`** [HTML](https://developer.mozilla.org/en-US/docs/Web/HTML) element represents a document section containing interactive controls for submitting information.\n\nFor more information, see the [documentation](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/form).",
		Attributes: map[string]schema.Attribute{
			...
			"accept_charset": schema.DynamicAttribute{
				MarkdownDescription: "Space-separated [character encodings](https://developer.mozilla.org/en-US/docs/Glossary/Character_encoding) the server accepts.",
				Optional:            true,
			},
			"autocapitalize": schema.DynamicAttribute{
				MarkdownDescription: "Controls whether inputted text is automatically capitalized and, if so, in what manner.",
				Optional:            true,
			},
			"autocomplete": schema.DynamicAttribute{
				MarkdownDescription: "Indicates whether input elements can by default have their values automatically completed by the browser.",
				Optional:            true,
			},
			"name": schema.DynamicAttribute{
				MarkdownDescription: "The name of the form. The value must not be the empty string, and must be unique among the `form` elements in the forms collection that it is in, if any.",
				Optional:            true,
			},
			"rel": schema.DynamicAttribute{
				MarkdownDescription: "Controls the annotations and what kinds of links the form creates.",
				Optional:            true,
			},
			"action": schema.DynamicAttribute{
				MarkdownDescription: "The URL that processes the form submission.",
				Optional:            true,
			},
			"enctype": schema.DynamicAttribute{
				MarkdownDescription: "If the value of the `method` attribute is `post`, `enctype` is the [MIME type](https://en.wikipedia.org/wiki/Mime_type) of the form submission.",
				Optional:            true,
			},
			"method": schema.DynamicAttribute{
				MarkdownDescription: "The [HTTP](https://developer.mozilla.org/en-US/docs/Web/HTTP) method to submit the form with.",
				Optional:            true,
			},
			"novalidate": schema.DynamicAttribute{
				MarkdownDescription: "This Boolean attribute indicates that the form shouldn't be validated when submitted.",
				Optional:            true,
			},
			"target": schema.DynamicAttribute{
				MarkdownDescription: "Indicates where to display the response after submitting the form. It is a name/keyword for a _browsing context_ (for example, tab, window, or iframe).",
				Optional:            true,
			},
...
type resource_formModel struct {
	Children        types.List    `tfsdk:"children"`
	Accept_charset  types.Dynamic `tfsdk:"accept_charset"`
	...
	Action          types.Dynamic `tfsdk:"action"`
	Enctype         types.Dynamic `tfsdk:"enctype"`
	Method          types.Dynamic `tfsdk:"method"`
	Novalidate      types.Dynamic `tfsdk:"novalidate"`
...
func (r *resource_form) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	r.handleRequest(ctx, &req.Plan, &resp.State, &resp.Diagnostics)
}

func (r *resource_form) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	r.handleRequest(ctx, &req.State, &resp.State, &resp.Diagnostics)
}

func (r *resource_form) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	r.handleRequest(ctx, &req.Plan, &resp.State, &resp.Diagnostics)
}

func (r *resource_form) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
}

func (r *resource_form) handleRequest(ctx context.Context, g util.ModelGetter, s util.ModelSetter, diags *diag.Diagnostics) {
	util.HandleRequest(
		ctx,
		&resource_formModel{},
		g,
		...
		func(m *resource_formModel) bool {
			...
			attrs := []string{}
			if !m.Accept_charset.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "accept-charset", m.Accept_charset)
				if err != nil {
					diags.AddError("invalid accept-charset attribute", err.Error())
					return false
				}
			...
			if !m.Action.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "action", m.Action)
				if err != nil {
					diags.AddError("invalid action attribute", err.Error())
					return false
				}
				attrs = append(attrs, attr)
			}
			if !m.Enctype.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "enctype", m.Enctype)
				if err != nil {
					diags.AddError("invalid enctype attribute", err.Error())
					return false
				}
				attrs = append(attrs, attr)
			}
			if !m.Method.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "method", m.Method)
				if err != nil {
					diags.AddError("invalid method attribute", err.Error())
					return false
				}
				attrs = append(attrs, attr)
			}
			if !m.Novalidate.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "novalidate", m.Novalidate)
				if err != nil {
					diags.AddError("invalid novalidate attribute", err.Error())
					return false
				}
				attrs = append(attrs, attr)
			}
