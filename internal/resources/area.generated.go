	...
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"alt": schema.DynamicAttribute{
				MarkdownDescription: "A text string alternative to display on browsers that do not display images.",
				Optional:            true,
			},
			"coords": schema.DynamicAttribute{
				MarkdownDescription: "The `coords` attribute details the coordinates of the [`shape`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/area#shape) attribute in size, shape, and placement of an `<area>`.",
				Optional:            true,
			},
			"download": schema.DynamicAttribute{
				MarkdownDescription: "This attribute, if present, indicates that the author intends the hyperlink to be used for downloading a resource.",
				Optional:            true,
			},
			"href": schema.DynamicAttribute{
				MarkdownDescription: "The hyperlink target for the area.",
				Optional:            true,
			},
			"ping": schema.DynamicAttribute{
				MarkdownDescription: "Contains a space-separated list of URLs to which, when the hyperlink is followed, [`POST`](https://developer.mozilla.org/en-US/docs/Web/HTTP/Methods/POST) requests with the body `PING` will be sent by the browser (in the background).",
				Optional:            true,
			},
			...
			"rel": schema.DynamicAttribute{
				MarkdownDescription: "For anchors containing the [`href`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/area#href) attribute, this attribute specifies the relationship of the target object to the link object.",
				Optional:            true,
			},
			"shape": schema.DynamicAttribute{
				MarkdownDescription: "The shape of the associated hot spot.",
				Optional:            true,
			},
			"target": schema.DynamicAttribute{
				MarkdownDescription: "A keyword or author-defined name of the [browsing context](https://developer.mozilla.org/en-US/docs/Glossary/Browsing_context) to display the linked resource.",
				Optional:            true,
			},
...
type resource_areaModel struct {
	...
	Shape           types.Dynamic `tfsdk:"shape"`
...
func (r *resource_area) handleRequest(ctx context.Context, g util.ModelGetter, s util.ModelSetter, diags *diag.Diagnostics) {
	util.HandleRequest(
		...
		func(m *resource_areaModel) bool {
			...
			if !m.Coords.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "coords", m.Coords)
				if err != nil {
					diags.AddError("invalid coords attribute", err.Error())
					return false
				}
				attrs = append(attrs, attr)
			}
			...
			if !m.Shape.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "shape", m.Shape)
				if err != nil {
					diags.AddError("invalid shape attribute", err.Error())
					return false
				}
				attrs = append(attrs, attr)
			}
