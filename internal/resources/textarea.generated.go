	...
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"autocomplete": schema.DynamicAttribute{
				MarkdownDescription: "This attribute indicates whether the value of the control can be automatically completed by the browser.",
				Optional:            true,
			},
			"autofocus": schema.DynamicAttribute{
				MarkdownDescription: "This Boolean attribute lets you specify that a form control should have input focus when the page loads.",
				Optional:            true,
			},
			"cols": schema.DynamicAttribute{
				MarkdownDescription: "The visible width of the text control, in average character widths.",
				Optional:            true,
			},
			"dirname": schema.DynamicAttribute{
				MarkdownDescription: "This attribute is used to indicate the text directionality of the element contents.",
				Optional:            true,
			},
			"disabled": schema.DynamicAttribute{
				MarkdownDescription: "This Boolean attribute indicates that the user cannot interact with the control.",
				Optional:            true,
			},
			"form": schema.DynamicAttribute{
				MarkdownDescription: "The form element that the `<textarea>` element is associated with (its \"form owner\").",
				Optional:            true,
			},
			"maxlength": schema.DynamicAttribute{
				MarkdownDescription: "The maximum string length (measured in UTF-16 code units) that the user can enter.",
				Optional:            true,
			},
			"minlength": schema.DynamicAttribute{
				MarkdownDescription: "The minimum string length (measured in UTF-16 code units) required that the user should enter.",
				Optional:            true,
			},
			"name": schema.DynamicAttribute{
				MarkdownDescription: "The name of the control.",
				Optional:            true,
			},
			"placeholder": schema.DynamicAttribute{
				MarkdownDescription: "A hint to the user of what can be entered in the control.",
				Optional:            true,
			},
			"readonly": schema.DynamicAttribute{
				MarkdownDescription: "This Boolean attribute indicates that the user cannot modify the value of the control.",
				Optional:            true,
			},
			"required": schema.DynamicAttribute{
				MarkdownDescription: "This attribute specifies that the user must fill in a value before submitting a form.",
				Optional:            true,
			},
			"rows": schema.DynamicAttribute{
				MarkdownDescription: "The number of visible text lines for the control.",
				Optional:            true,
			},
			"spellcheck": schema.DynamicAttribute{
				MarkdownDescription: "Specifies whether the `<textarea>` is subject to spell checking by the underlying browser/OS.",
				Optional:            true,
			},
			"wrap": schema.DynamicAttribute{
				MarkdownDescription: "Indicates how the control should wrap the value for form submission.",
				Optional:            true,
			},
...
func (r *resource_textarea) handleRequest(ctx context.Context, g util.ModelGetter, s util.ModelSetter, diags *diag.Diagnostics) {
	util.HandleRequest(
		...
		func(m *resource_textareaModel) bool {
			...
			if !m.Cols.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "cols", m.Cols)
				if err != nil {
					diags.AddError("invalid cols attribute", err.Error())
					return false
				}
				attrs = append(attrs, attr)
			}
			...
			if !m.Rows.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "rows", m.Rows)
				if err != nil {
					diags.AddError("invalid rows attribute", err.Error())
					return false
				}
				attrs = append(attrs, attr)
			}
			...
			if !m.Wrap.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "wrap", m.Wrap)
				if err != nil {
					diags.AddError("invalid wrap attribute", err.Error())
					return false
				}
				attrs = append(attrs, attr)
			}
