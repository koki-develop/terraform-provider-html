	...
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"data": schema.DynamicAttribute{
				MarkdownDescription: "The address of the resource as a valid URL. At least one of **data** and **type** must be defined.",
				Optional:            true,
			},
			"form": schema.DynamicAttribute{
				MarkdownDescription: "The form element, if any, that the object element is associated with (its _form owner_).",
				Optional:            true,
			},
			"height": schema.DynamicAttribute{
				MarkdownDescription: "The height of the displayed resource, in [CSS pixels](https://drafts.csswg.org/css-values/#px).",
				Optional:            true,
			},
			"name": schema.DynamicAttribute{
				MarkdownDescription: "The name of valid browsing context (HTML5), or the name of the control (HTML 4).",
				Optional:            true,
			},
			"type": schema.DynamicAttribute{
				MarkdownDescription: "The [content type](https://developer.mozilla.org/en-US/docs/Glossary/MIME_type) of the resource specified by **data**.",
				Optional:            true,
			},
			"width": schema.DynamicAttribute{
				MarkdownDescription: "The width of the display resource, in [CSS pixels](https://drafts.csswg.org/css-values/#px).",
				Optional:            true,
			},
...
func (r *resource_object) handleRequest(ctx context.Context, g util.ModelGetter, s util.ModelSetter, diags *diag.Diagnostics) {
	util.HandleRequest(
		...
		func(m *resource_objectModel) bool {
			...
			attrs := []string{}
			if !m.Data.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "data", m.Data)
				if err != nil {
					diags.AddError("invalid data attribute", err.Error())
					return false
				}
