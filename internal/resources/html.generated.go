...
func (r *resource_html) Schema(_ context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"xmlns": schema.DynamicAttribute{
				MarkdownDescription: "Specifies the [XML](https://developer.mozilla.org/en-US/docs/Glossary/XML) [Namespace](https://developer.mozilla.org/en-US/docs/Glossary/Namespace) of the document.",
				Optional:            true,
			},
...
func (r *resource_html) handleRequest(ctx context.Context, g util.ModelGetter, s util.ModelSetter, diags *diag.Diagnostics) {
	util.HandleRequest(
		...
		func(m *resource_htmlModel) bool {
			...
			attrs := []string{}
			if !m.Xmlns.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "xmlns", m.Xmlns)
				if err != nil {
					diags.AddError("invalid xmlns attribute", err.Error())
					return false
				}
