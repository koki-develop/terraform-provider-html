	...
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"cite": schema.DynamicAttribute{
				MarkdownDescription: "This attribute defines the URI of a resource that explains the change, such as a link to meeting minutes or a ticket in a troubleshooting system.",
				Optional:            true,
			},
			"datetime": schema.DynamicAttribute{
				MarkdownDescription: "This attribute indicates the time and date of the change and must be a valid date with an optional time string.",
				Optional:            true,
			},
	...
	util.HandleRequest(
		...
		func(m *resource_insModel) bool {
			...
			if !m.Datetime.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "datetime", m.Datetime)
				if err != nil {
					diags.AddError("invalid datetime attribute", err.Error())
					return false
				}
				attrs = append(attrs, attr)
			}
