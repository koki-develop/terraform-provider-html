...
func (r *resource_meta) Schema(_ context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"charset": schema.DynamicAttribute{
				MarkdownDescription: "This attribute declares the document's character encoding.",
				Optional:            true,
			},
			"content": schema.DynamicAttribute{
				MarkdownDescription: "This attribute contains the value for the [`http-equiv`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/meta#http-equiv) or [`name`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/meta#name) attribute, depending on which is used.",
				Optional:            true,
			},
			"http_equiv": schema.DynamicAttribute{
				MarkdownDescription: "Defines a pragma directive.",
				Optional:            true,
			},
			"name": schema.DynamicAttribute{
				MarkdownDescription: "The `name` and `content` attributes can be used together to provide document metadata in terms of name-value pairs, with the `name` attribute giving the metadata name, and the `content` attribute giving the value.",
				Optional:            true,
			},
...
type resource_metaModel struct {
	...
	Http_equiv      types.Dynamic `tfsdk:"http_equiv"`
...
func (r *resource_meta) handleRequest(ctx context.Context, g util.ModelGetter, s util.ModelSetter, diags *diag.Diagnostics) {
	util.HandleRequest(
		...
		func(m *resource_metaModel) bool {
			...
			attrs := []string{}
			if !m.Charset.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "charset", m.Charset)
				if err != nil {
					diags.AddError("invalid charset attribute", err.Error())
					return false
				}
				attrs = append(attrs, attr)
			}
			if !m.Content.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "content", m.Content)
				if err != nil {
					diags.AddError("invalid content attribute", err.Error())
					return false
				}
				attrs = append(attrs, attr)
			}
			if !m.Http_equiv.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "http-equiv", m.Http_equiv)
				if err != nil {
					diags.AddError("invalid http-equiv attribute", err.Error())
					return false
				}
				attrs = append(attrs, attr)
			}
