...
var (
	_ resource.Resource = &resource_output{}
)

func newResource_output() resource.Resource {
	return &resource_output{}
}

type resource_output struct{}

func (r *resource_output) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_output"
}

func (r *resource_output) Schema(_ context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "The **`<output>`** [HTML](https://developer.mozilla.org/en-US/docs/Web/HTML) element is a container element into which a site or app can inject the results of a calculation or the outcome of a user action.\n\nFor more information, see the [documentation](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/output).",
		Attributes: map[string]schema.Attribute{
			...
			"for": schema.DynamicAttribute{
				MarkdownDescription: "A space-separated list of other elements' [`id`](https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes#id)s, indicating that those elements contributed input values to (or otherwise affected) the calculation.",
				Optional:            true,
			},
			"form": schema.DynamicAttribute{
				MarkdownDescription: "The [`<form>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/form) element to associate the output with (its _form owner_).",
				Optional:            true,
			},
			"name": schema.DynamicAttribute{
				MarkdownDescription: "The element's name.",
				Optional:            true,
			},
...
type resource_outputModel struct {
	Children        types.List    `tfsdk:"children"`
	For             types.Dynamic `tfsdk:"for"`
...
func (r *resource_output) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	r.handleRequest(ctx, &req.Plan, &resp.State, &resp.Diagnostics)
}

func (r *resource_output) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	r.handleRequest(ctx, &req.State, &resp.State, &resp.Diagnostics)
}

func (r *resource_output) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	r.handleRequest(ctx, &req.Plan, &resp.State, &resp.Diagnostics)
}

func (r *resource_output) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
}

func (r *resource_output) handleRequest(ctx context.Context, g util.ModelGetter, s util.ModelSetter, diags *diag.Diagnostics) {
	util.HandleRequest(
		ctx,
		&resource_outputModel{},
		g,
		s,
		diags,
		func(m *resource_outputModel) bool {
			html := new(strings.Builder)
			html.WriteString("<output")

			attrs := []string{}
			if !m.For.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "for", m.For)
				if err != nil {
					diags.AddError("invalid for attribute", err.Error())
					return false
				}
