...
func (d *datasource_template) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"shadowrootmode": schema.DynamicAttribute{
				MarkdownDescription: "Creates a [shadow root](https://developer.mozilla.org/en-US/docs/Glossary/Shadow_tree) for the parent element.",
				Optional:            true,
			},
			"shadowrootclonable": schema.DynamicAttribute{
				MarkdownDescription: "Sets the value of the [`clonable`](https://developer.mozilla.org/en-US/docs/Web/API/ShadowRoot/clonable) property of a [`ShadowRoot`](https://developer.mozilla.org/en-US/docs/Web/API/ShadowRoot) created using this element to `true`.",
				Optional:            true,
			},
			"shadowrootdelegatesfocus": schema.DynamicAttribute{
				MarkdownDescription: "Sets the value of the [`delegatesFocus`](https://developer.mozilla.org/en-US/docs/Web/API/ShadowRoot/delegatesFocus) property of a [`ShadowRoot`](https://developer.mozilla.org/en-US/docs/Web/API/ShadowRoot) created using this element to `true`.",
				Optional:            true,
			},
...
type datasource_templateModel struct {
	...
	Shadowrootdelegatesfocus types.Dynamic `tfsdk:"shadowrootdelegatesfocus"`
...
func (d *datasource_template) handleRequest(ctx context.Context, g util.ModelGetter, s util.ModelSetter, diags *diag.Diagnostics) {
	util.HandleRequest(
		...
		func(m *datasource_templateModel) bool {
			...
			attrs := []string{}
			if !m.Shadowrootmode.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "shadowrootmode", m.Shadowrootmode)
				if err != nil {
					diags.AddError("invalid shadowrootmode attribute", err.Error())
					return false
				}
				attrs = append(attrs, attr)
			}
			if !m.Shadowrootclonable.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "shadowrootclonable", m.Shadowrootclonable)
				if err != nil {
					diags.AddError("invalid shadowrootclonable attribute", err.Error())
					return false
				}
				attrs = append(attrs, attr)
			}
			if !m.Shadowrootdelegatesfocus.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "shadowrootdelegatesfocus", m.Shadowrootdelegatesfocus)
				if err != nil {
					diags.AddError("invalid shadowrootdelegatesfocus attribute", err.Error())
					return false
				}
				attrs = append(attrs, attr)
			}
