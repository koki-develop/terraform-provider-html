	...
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"disabled": schema.DynamicAttribute{
				MarkdownDescription: "If this Boolean attribute is set, this option is not checkable.",
				Optional:            true,
			},
			"label": schema.DynamicAttribute{
				MarkdownDescription: "This attribute is text for the label indicating the meaning of the option.",
				Optional:            true,
			},
			"selected": schema.DynamicAttribute{
				MarkdownDescription: "If present, this Boolean attribute indicates that the option is initially selected.",
				Optional:            true,
			},
			"value": schema.DynamicAttribute{
				MarkdownDescription: "The content of this attribute represents the value to be submitted with the form, should this option be selected.",
				Optional:            true,
			},
...
type resource_optionModel struct {
	...
	Selected        types.Dynamic `tfsdk:"selected"`
...
func (r *resource_option) handleRequest(ctx context.Context, g util.ModelGetter, s util.ModelSetter, diags *diag.Diagnostics) {
	util.HandleRequest(
		...
		func(m *resource_optionModel) bool {
			...
			if !m.Selected.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "selected", m.Selected)
				if err != nil {
					diags.AddError("invalid selected attribute", err.Error())
					return false
				}
				attrs = append(attrs, attr)
			}
