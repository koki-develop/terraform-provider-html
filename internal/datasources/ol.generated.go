...
func (d *datasource_ol) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"reversed": schema.DynamicAttribute{
				MarkdownDescription: "This Boolean attribute specifies that the list's items are in reverse order.",
				Optional:            true,
			},
			"start": schema.DynamicAttribute{
				MarkdownDescription: "An integer to start counting from for the list items.",
				Optional:            true,
			},
			"type": schema.DynamicAttribute{
				MarkdownDescription: "Sets the numbering type.",
				Optional:            true,
			},
...
func (d *datasource_ol) handleRequest(ctx context.Context, g util.ModelGetter, s util.ModelSetter, diags *diag.Diagnostics) {
	util.HandleRequest(
		...
		func(m *datasource_olModel) bool {
			...
			attrs := []string{}
			if !m.Reversed.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "reversed", m.Reversed)
				if err != nil {
					diags.AddError("invalid reversed attribute", err.Error())
					return false
				}
				attrs = append(attrs, attr)
			}
			if !m.Start.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "start", m.Start)
				if err != nil {
					diags.AddError("invalid start attribute", err.Error())
					return false
				}
				attrs = append(attrs, attr)
			}
