...
func (d *datasource_iframe) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"allow": schema.DynamicAttribute{
				MarkdownDescription: "Specifies a [Permissions Policy](https://developer.mozilla.org/en-US/docs/Web/HTTP/Permissions_Policy) for the `<iframe>`.",
				Optional:            true,
			},
			"allowfullscreen": schema.DynamicAttribute{
				MarkdownDescription: "Set to `true` if the `<iframe>` can activate fullscreen mode by calling the [`requestFullscreen()`](https://developer.mozilla.org/en-US/docs/Web/API/Element/requestFullscreen \"requestFullscreen()\") method.",
				Optional:            true,
			},
			"height": schema.DynamicAttribute{
				MarkdownDescription: "The height of the frame in CSS pixels.",
				Optional:            true,
			},
			"loading": schema.DynamicAttribute{
				MarkdownDescription: "Indicates when the browser should load the iframe.",
				Optional:            true,
			},
			"name": schema.DynamicAttribute{
				MarkdownDescription: "A targetable name for the embedded browsing context.",
				Optional:            true,
			},
			"referrerpolicy": schema.DynamicAttribute{
				MarkdownDescription: "Indicates which [referrer](https://developer.mozilla.org/en-US/docs/Web/API/Document/referrer) to send when fetching the frame's resource.",
				Optional:            true,
			},
			"sandbox": schema.DynamicAttribute{
				MarkdownDescription: "Controls the restrictions applied to the content embedded in the `<iframe>`.",
				Optional:            true,
			},
			"src": schema.DynamicAttribute{
				MarkdownDescription: "The URL of the page to embed. Use a value of `about:blank` to embed an empty page that conforms to the [same-origin policy](https://developer.mozilla.org/en-US/docs/Web/Security/Same-origin_policy#inherited_origins).",
				Optional:            true,
			},
			"srcdoc": schema.DynamicAttribute{
				MarkdownDescription: "Inline HTML to embed, overriding the `src` attribute.",
				Optional:            true,
			},
			"width": schema.DynamicAttribute{
				MarkdownDescription: "The width of the frame in CSS pixels.",
				Optional:            true,
			},
...
type datasource_iframeModel struct {
	...
	Sandbox         types.Dynamic `tfsdk:"sandbox"`
	Src             types.Dynamic `tfsdk:"src"`
	Srcdoc          types.Dynamic `tfsdk:"srcdoc"`
...
func (d *datasource_iframe) handleRequest(ctx context.Context, g util.ModelGetter, s util.ModelSetter, diags *diag.Diagnostics) {
	util.HandleRequest(
		...
		func(m *datasource_iframeModel) bool {
			...
			attrs := []string{}
			if !m.Allow.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "allow", m.Allow)
				if err != nil {
					diags.AddError("invalid allow attribute", err.Error())
					return false
				}
				attrs = append(attrs, attr)
			}
			if !m.Allowfullscreen.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "allowfullscreen", m.Allowfullscreen)
				if err != nil {
					diags.AddError("invalid allowfullscreen attribute", err.Error())
					return false
				}
				attrs = append(attrs, attr)
			}
			...
			if !m.Sandbox.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "sandbox", m.Sandbox)
				if err != nil {
					diags.AddError("invalid sandbox attribute", err.Error())
					return false
				}
				attrs = append(attrs, attr)
			}
			...
			if !m.Srcdoc.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "srcdoc", m.Srcdoc)
				if err != nil {
					diags.AddError("invalid srcdoc attribute", err.Error())
					return false
				}
				attrs = append(attrs, attr)
			}
