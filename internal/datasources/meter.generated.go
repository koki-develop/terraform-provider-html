...
var (
	_ resource.Resource = &resource_meter{}
)

func newResource_meter() resource.Resource {
	return &resource_meter{}
}

type resource_meter struct{}

func (r *resource_meter) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_meter"
}

func (r *resource_meter) Schema(_ context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "The **`<meter>`** [HTML](https://developer.mozilla.org/en-US/docs/Web/HTML) element represents either a scalar value within a known range or a fractional value.\n\nFor more information, see the [documentation](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/meter).",
		Attributes: map[string]schema.Attribute{
			...
			"value": schema.DynamicAttribute{
				MarkdownDescription: "The current numeric value.",
				Optional:            true,
			},
			"min": schema.DynamicAttribute{
				MarkdownDescription: "The lower numeric bound of the measured range.",
				Optional:            true,
			},
			"max": schema.DynamicAttribute{
				MarkdownDescription: "The upper numeric bound of the measured range.",
				Optional:            true,
			},
			"low": schema.DynamicAttribute{
				MarkdownDescription: "The upper numeric bound of the low end of the measured range.",
				Optional:            true,
			},
			"high": schema.DynamicAttribute{
				MarkdownDescription: "The lower numeric bound of the high end of the measured range.",
				Optional:            true,
			},
			"optimum": schema.DynamicAttribute{
				MarkdownDescription: "This attribute indicates the optimal numeric value.",
				Optional:            true,
			},
			"form": schema.DynamicAttribute{
				MarkdownDescription: "This optional attribute is used to explicitly set a [`<form>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/form) owner for the `<meter>` element.",
				Optional:            true,
			},
...
type resource_meterModel struct {
	Children        types.List    `tfsdk:"children"`
	Value           types.Dynamic `tfsdk:"value"`
	Min             types.Dynamic `tfsdk:"min"`
	Max             types.Dynamic `tfsdk:"max"`
	Low             types.Dynamic `tfsdk:"low"`
	High            types.Dynamic `tfsdk:"high"`
	Optimum         types.Dynamic `tfsdk:"optimum"`
...
func (r *resource_meter) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	r.handleRequest(ctx, &req.Plan, &resp.State, &resp.Diagnostics)
}

func (r *resource_meter) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	r.handleRequest(ctx, &req.State, &resp.State, &resp.Diagnostics)
}

func (r *resource_meter) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	r.handleRequest(ctx, &req.Plan, &resp.State, &resp.Diagnostics)
}

func (r *resource_meter) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
}

func (r *resource_meter) handleRequest(ctx context.Context, g util.ModelGetter, s util.ModelSetter, diags *diag.Diagnostics) {
	util.HandleRequest(
		ctx,
		&resource_meterModel{},
		g,
		...
		func(m *resource_meterModel) bool {
			...
			if !m.Low.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "low", m.Low)
				if err != nil {
					diags.AddError("invalid low attribute", err.Error())
					return false
				}
				attrs = append(attrs, attr)
			}
			if !m.High.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "high", m.High)
				if err != nil {
					diags.AddError("invalid high attribute", err.Error())
					return false
				}
				attrs = append(attrs, attr)
			}
			if !m.Optimum.IsNull() {
				attr, err := util.StringifyAttribute(ctx, "optimum", m.Optimum)
				if err != nil {
					diags.AddError("invalid optimum attribute", err.Error())
					return false
				}
				attrs = append(attrs, attr)
			}
